...
<Steps>
  ...
  <Step title="Clip loads into the timeline">
    The engine parses the VMD, and all bone and morph tracks appear in the dopesheet immediately. The playhead resets to frame 0 and any previous animation is replaced. The undo/redo history is **not** cleared — the load is treated as an external replacement and does not go on the history stack.
  </Step>
