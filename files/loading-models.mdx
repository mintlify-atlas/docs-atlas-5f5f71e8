...
5. Recalculates `frameCount` so it is at least as long as the latest retained keyframe.

The clip display name and the playhead position are preserved. If the retained clip would be completely empty (no tracks at all), a fresh blank clip is created instead and the clip display name resets to the model's filename stem.
